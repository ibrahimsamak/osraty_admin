...
@Component({
  selector: 'ngx-buyunits',
  templateUrl: './buyunits.component.html',
  styleUrls: ['./buyunits.component.scss']
})
export class BuyunitsComponent implements OnInit {
  posts :any;
  settings = {
    add: {
  ...
  getData(){
   this.subscripe =  this.service.getBuyUnitsData().subscribe(userList => {
  ...
  onDeleteConfirm(event): void 
  {
    if (window.confirm('هل أنت متأكد من حذف العنصر؟')) 
    {
      ...
      this.service.DeleteBuyUnitsData(event.data._id).subscribe(x=>{
        console.log(x);
  ...
  onEdit(event) 
  {
   let conent = {
    ...
    this.service.UpdateBuyUnitsData(event.data._id,conent).subscribe(x=>{
      console.log(x);
  ...
  onCreate(event) 
  {
   let conent = {
    ...
    this.service.CreateBuyUnitsData(conent).subscribe(x=>{
      console.log(x);
      event.confirm.resolve();
      this.getData();
    });
