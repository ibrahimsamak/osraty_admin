...
export class NetworkComponent implements OnInit {
  ...
  getData() {
    this.subscripe = this.service.getpaymentforData().subscribe(userList => {
  ...
  onDeleteConfirm(event): void {
    if (window.confirm('هل أنت متأكد من حذف العنصر؟')) {
      ...
      this.service.DeletepaymentforData(event.data._id).subscribe(x => {
        console.log(x);
  ...
  onEdit(event) {
    ...
    this.service.UpdatepaymentforData(event.data._id, conent).subscribe(x => {
      console.log(x);
  ...
  onCreate(event) {
    ...
    this.service.CreatepaymentforData(conent).subscribe(x => {
      console.log(x);
      event.confirm.resolve();
      this.getData();
    });
