...
export class JobsComponent implements OnInit {
  ...
  onDeleteConfirm(event): void {
    if (window.confirm('هل أنت متأكد من حذف العنصر؟')) {
      ...
      this.service.DeleteJobssData(event.data._id).subscribe(x => {
        console.log(x);
  ...
  onEdit(event) {
    ...
    this.service.UpdateJobssData(event.data._id, conent).subscribe(x => {
      console.log(x);
  ...
  onCreate(event) {
    ...
    this.service.CreateJobsData(conent).subscribe(x => {
      console.log(x);
      event.confirm.resolve();
      this.getData();
    });
