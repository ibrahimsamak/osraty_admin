...
export class SociallinksComponent implements OnInit {
  ...
  getData(){
   this.subscripe =  this.service.getSocialOptionData().subscribe(userList => {
  ...
  onDeleteConfirm(event): void 
  {
    if (window.confirm('هل أنت متأكد من حذف العنصر؟')) 
    {
      ...
      this.service.DeleteSocialOptionData(event.data._id).subscribe(x=>{
        console.log(x);
  ...
  onEdit(event) 
  {
   let conent = {
    ...
    this.service.UpdateSocialOptionData(event.data._id,conent).subscribe(x=>{
      console.log(x);
  ...
  onCreate(event) 
  {
   let conent = {
    ...
    this.service.CreateSocialOptionData(conent).subscribe(x=>{
      console.log(x);
      event.confirm.resolve();
      this.getData();
    });
