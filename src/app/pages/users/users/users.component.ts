...
export class UsersComponent extends SuperComponent implements OnInit {
  ...
  setPage(pageInfo) {
    ...
    this.service.getUserWithPaginationData(this.page.pageNumber).subscribe(pagedData => {
  ...
  updateFilter(event) {
    ...
    if (val.value != '') {
      ...
      this.service.getSearchUserData(content, 0).subscribe((res) => {
  ...
  Block(event) {
    if (window.confirm('هل أنت متأكد من حذف العنصر؟')) {
      if (event.isBlock == true) {
        ...
        this.service.blockUser(content, event._id).subscribe(response => {
          console.log(response)
          this.showToast('success', 'نجاح', 'تم الغاء حظر المستفيد');
        })
      } else {
        // block
        let content = {
        ...
        this.service.blockUser(content, event._id).subscribe(response => {
          console.log(response)
          this.showToast('success', 'نجاح', 'تم حظر المستفيد');
        })
      }
    }
  }
  ...
  history(event) {
    ...
    this.router.navigate(['/pages/report/history/', { id: event['_id'], user_name: event['full_name'], type: 'user' }]);
  }
